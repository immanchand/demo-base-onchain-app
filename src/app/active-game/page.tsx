          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="col-span-1 text-right relative group">
              <p className="text-primary-text">
                ...
                {isCopied && (
                  <span className="absolute left-1/2 -translate-x-1/2 top-full mt-1 text-accent-yellow text-xs animate-fade-in-out">
                    COPIED!
                  </span>
                )}
              </p>
    ...
    try {
      ...
      if (gameId > 0) {
        ...
        if (gameData.endTime <= currentTime || gameData.error) {
          gameData = null; // Trigger creation if game is over or errored
        }
      }
  ...
  const handleCreateGameStatusChange = useCallback(
    async (status: 'idle' | 'pending' | 'success' | 'error', message?: string) => {
      if (status === 'success' && message?.startsWith('Transaction hash:')) {
        ...
        // Fetching happens in initializeGameFlow, no need to duplicate here
              ...
              <section className="flex w-full flex-col items-center gap-4 px-2 py-4">
                <div className="flex w-full justify-center gap-4 mb-4">
                  <button
                    ...
                    className={` ${selectedGame === 'space-invaders' ? 'btn-menu-selected' : 'btn-menu-idle'}`}
                  >
                    SPACE INVADERS
                  </button>
                  ...
                  <button
                    ...
                    className={` ${selectedGame === 'asteroids' ? 'btn-menu-selected' : 'btn-menu-idle'}`}
                  >
                    ASTEROIDS
                  </button>
                </div>
                ...
                {selectedGame === 'asteroids' && (
                  <div className="w-full">
                    <Asteroids gameId={Number(gameState.game.gameId)} existingHighScore={Number(gameState.game.highScore)} updateTickets={refreshTickets} />
                  </div>
                )}
              </section>
            ) : (
